...
`playwright.config.ts` builds with `VITE_ENABLE_E2E_ACCOUNTS=true`, serves the production build, and runs six projects: the app specs (`smoke` + `interactions` + `persistence`) once per engine as `app` (Chromium), `app-firefox` and `app-webkit`, plus `role-employee` / `role-manager` / `role-admin` (each runs `roles.spec.ts` for its persona, in parallel). `global-setup.ts` provisions the Firebase Auth test accounts; personas/credentials are in `tests/e2e/config.ts` and overridable via `E2E_*` env vars.
