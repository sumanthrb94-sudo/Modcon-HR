...
const APP_SPECS = /(smoke|interactions|persistence|attendance|regularizations|check-in-out|provisioning|password-reset|no-auto-signin|leave-balance|payslip-upload|document-upload-access|reporting-manager|form-capture|helpdesk-attribution|employee-counters|auth-control-label|back-to-back-writes|tab-isolation)\.spec\.ts$/;
 ...
 */
const SHARED_CONFIG_SPECS = /(org-settings|salary-structure|employee-leave-policy|leave-unpaid-fallback|location-directory|hr-designations|onboarding|careers|shift-timings|role-change-propagation|week-off-policy|geofenced-attendance|shared-records|statutory-payroll|payroll-run-guard|org-create-validation|superadmin-governance|approvals-queue-scope|write-failure-rollback|support-chat)\.spec\.ts$/;
